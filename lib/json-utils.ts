...
export function toTimeSeries(data: any, xPath: string, yPath: string) {
  ...
  if (
    data &&
    typeof data === "object" &&
    ["o", "h", "l", "c"].every((k) => k in data)
  ) {
    const ts = Number.isFinite(Number(data.t))
      ? Number(data.t) * 1000
      : Date.now();
    const row = {
    ...
    return [{ time: row.time, value: Number(pick(yPath)) }].filter((r) =>
  ...
  const seriesKey =
    data &&
    Object.keys(data).find((k) => k.toLowerCase().includes("time series"));
  if (seriesKey) {
    const obj = data[seriesKey];
    const rows = Object.entries<any>(obj).map(([time, ohlc]) => ({
      time,
      value: Number(
        getByPath(normalizeOHLC(ohlc), yPath) ?? getByPath(ohlc, yPath)
      ),
    }));
  ...
  if (arr) {
    return arr
      .map((row: any) => ({
        time: String(getByPath(row, xPath)),
        value: Number(getByPath(row, yPath)),
      }))
